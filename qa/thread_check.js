...
/* S0: 現在の事実 + 承認済み画像 1 回（cue は無い） */
{
  ...
  check((contentCode.match(/\.\/assets\//g) || []).length === 3, 'content references Koenji and licensed Inokashira / Suzunari photos only');
   ...
   深い data 検査（逐語 copy / 語彙 / Yaguchi / 外部 destination）は qa/works_check.js。 */
{
  ...
  check(CONTENT.threads.length === 5 && CONTENT.threads[0] === thread, 'KOENJI stays first; five threads including PARKS and LADY JANE');
  if (book && film) {
