...

{
  ...
  check(analytics.includes(".wk-work[data-work]") && analytics.includes("api.worksSectionView(el.getAttribute('data-work') || '')") && analytics.includes("var WORK_ENTRY_IDS = { book: 'book', film: 'film', music: 'music', video: 'video' };") && !analytics.includes("api.entryOpen('work', 'works')") && !analytics.includes('works: true') && !/wk-object|wk-category|textContent|innerText|data-video-id/.test(analytics), 'analytics-v3.js measures Works by per-section bounded ids (book / film / music / video) and reach only; no landing entry');
