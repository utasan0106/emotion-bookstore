...
assert.ok(photos.length>=1);for(const p of photos)assert.ok(fs.statSync(path.join(root,p)).size>1000,p);
assert.doesNotMatch(home,/home-encounter|data-city-scene-image|hc-culture-art|hc-hero-cta/);
require('./home_discovery_check');
