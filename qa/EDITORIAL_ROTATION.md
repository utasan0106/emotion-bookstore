...
実測したカレンダー（2026-09-10 に `release_content.js` と突き合わせて更新）:

| 日時 (JST) | 東京 | 高円寺 | 下北沢 | 神保町 |
|---|---|---|---|---|
| 〜2026-09-27 23:59 | 開 | 開 | 開 | 開 |
| 2026-09-28 00:00〜 | 開 | 開 | **閉** | 開 |
| 2026-10-07 00:00〜 | 開 | 開 | **閉** | **閉** |
...
jinbocho/jinbocho-theater-joyu-damashii-2026   2026-10-07T00:00:00+09:00
```

**2026-09-10 訂正。** ここには `jinbocho-theater-mizoguchi-2026`（9/11 期限）と書いてあったが、
神保町はすでに『女優魂2026』へ差し替わっており、期限は 10/07 だった。
「9/12 に神保町が閉じる」と読める表になっていたので直した。
この文書は本番の実データではないため、**差し替えのたびに `release_content.js` と
突き合わせること。** 次に閉じるのは下北沢（9/28）である。
