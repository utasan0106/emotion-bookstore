...
(async()=>{
 ...
 assert.equal(scene['[data-city-scene-image]'].attributes.src,undefined,'Changing weather city must not replace the cover drawing');
 assert.equal(scene['.hc-hero-cta'].attributes.href,undefined,'Changing weather must not move the works destination');
 assert.equal(rain.nodes['[data-weather-icon]'].textContent,'☂');assert.equal(rain.motion,undefined,'No particle animation control');
 rain.setFail(true);rain.advance(16*60000);rain.intervals[0]();await rain.flush();
 assert.equal(rain.body.dataset.cityWeather,undefined);
 assert.equal(scene['[data-city-scene-image]'].attributes.src,undefined,'Weather outage must not touch the cover');
 ...
 console.log('WEATHER_CLIENT_GO: close/reopen, focus, dismissed refresh, city choice, forecast wording, stable cover and destination, outage, expiry, visibility, detail exclusion and request privacy');
