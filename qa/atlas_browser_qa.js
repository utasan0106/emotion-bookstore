...
const ATLAS_STATE = () => ({
  ...
  cspv: window.__cspv, perm: window.__perm, writes: window.__writes, ls: localStorage.length, ss: sessionStorage.length, dataLayer: typeof window.dataLayer, gtag: typeof window.gtag,
...
(async () => {
  ...
  for (const v of VIEWPORTS) {
    ...
    check(S, 'no_overflow_no_player_no_input_no_storage_no_geolocation', !st.overflow && st.iframes === 0 && st.inputs === 0 && st.writes === 0 && st.ls === 0 && st.ss === 0 && st.perm.length === 0 && st.scripts.join() === './app.js,../analytics-v3.js' && st.dataLayer === 'undefined' && st.gtag === 'undefined', { overflow: st.overflow, inputs: st.inputs, writes: st.writes, perm: st.perm, scripts: st.scripts, dataLayer: st.dataLayer });
