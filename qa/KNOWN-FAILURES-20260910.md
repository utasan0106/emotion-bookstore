...
**同日訂正：最初の版は2件を誤って挙げた。** `content="noindex"` の完全一致で
探していたので、`noindex,nofollow`（51ページ）と `noindex, follow`（2ページ）を
見落としていた。robots の値をカンマで分けて見るように直し、例外の一覧も消した。
...
**2026-09-11 追記：直した。** リンクにするかどうかを分けた。同じ行き先が既に
そのページの行動リンクにあるときだけ、出典表示を文字だけにする。credit の文は
そのまま残るので出どころは消えていない。行き先も一行下にある。

ただし `works_browser_qa` は**次の assert で落ちる。** 重複で止まっていて見えていなかった。

```
AssertionError: works.html: main target smaller than 44px
```

`main` 内のリンクの当たり判定を44px以上と要求している。実測では `wk-list` の作品名が
27px、表紙・映像の出典リンクが15〜38px、「作品と街のつながりを読む →」が19px。
**5ページ全部にまたがる。** 直せば行間と余白が変わるので、見た目の判断が要る。
WCAG 2.2 の AA は 24×24 で、27px はそちらは満たす。44px は AAA 相当の線である。
これも `main` で落ちる。
