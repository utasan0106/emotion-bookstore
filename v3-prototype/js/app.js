 ...
 * network 送信 0 / analytics 0 / 外部 AI 0 / login 0。
 * ========================================================================== */
(function (global) {
  'use strict';
  ...
  function metaLine(experience) {
    return [experience.type, experience.duration, experience.price].join(' ／ ');
  }
  ...
  function surfaceUnderstanding() {
    ...
    nodes.push(h('p', { class: 'body-lg', text: 'この言葉の隣に、3つの体験を置いています。' }));
    nodes.push(h('div', { class: 'actions' }, [
      h('button', {
        class: 'btn btn-primary', type: 'button',
        onclick: function () {
          startDeck('base', P.baseDeck(state.recentIds));
          persist();
          go('discovery');
        }
      }, [h('span', { text: '3つ見てみる' })])
  ...
  function surfaceLegacyDiscovery(deck, experience, counter) {
    var nodes = [
      ...
      h('p', { class: 'note', text: personalizedExplanation(deck.facets) })
    ];
    var card = experienceCard(experience, counter);
    attachSwipe(card,
      function () { decide(experience.id, 'pass'); },
      function () { decide(experience.id, 'keep'); });
    ...
    nodes.push(h('div', { class: 'decision' }, [
      h('button', {
        ...
        onclick: function () { decide(experience.id, 'pass'); }
      }, [h('span', { text: '今回は違う' })]),
      h('button', {
        class: 'btn btn-solid', type: 'button',
        onclick: function () { decide(experience.id, 'keep'); }
      }, [h('span', { text: '気になる' })])
    ]));
    if (deck.index > 0) {
  ...
  function externalPrototypeStub(label) {
    /* PROTOTYPE_EXTERNAL_LINK_STUB — no href, URL, fetch, navigation, or network. */
    announce(label + 'はPrototypeでは開きません。');
  }
  ...
  function mobileDiscoveryCard(fixture, recordId) {
    ...
    if (fixture.slot === 'cafe') {
      content.push(h('button', {
        ...
        onclick: function () { externalPrototypeStub('お店の情報'); }
      }, [
        h('span', { class: 'discovery-inline-icon', 'aria-hidden': 'true', text: '↗' }),
        h('span', { text: 'お店の情報を見る（公式サイト）' }),
  ...
  function desktopDiscoveryMain(fixture, fixtureIndex, galleryIndex, onGallery, onKeep) {
    ...
    if (fixture.slot === 'cafe') {
      actions.push(h('button', {
        ...
        onclick: function () { externalPrototypeStub('公式サイト'); }
      }, [h('span', { text: '公式サイトを見る' }), h('span', { 'aria-hidden': 'true', text: '↗' })]));
      actions.push(h('button', {
        class: 'w03-link-button', type: 'button',
        onclick: function () { externalPrototypeStub('Googleマップ'); }
      }, [h('span', { text: 'Googleマップで見る' }), h('span', { 'aria-hidden': 'true', text: '↗' })]));
    }
    actions.push(h('button', {
  ...
  function surfaceDiscovery() {
    ...
    if (deck.mode === 'personalized') return surfaceLegacyDiscovery(deck, experience, counter);
  ...
  function legacyJudgementRow(id) {
    ...
    var controls = [
      h('button', {
        ...
        'aria-pressed': value === 'keep' ? 'true' : 'false',
        onclick: function () {
  ...
  function surfaceNone() {
    return section('04-discovery-none', [
      h('h1', { class: 'question', tabindex: '-1', id: 'surface-title', text: '今日は、この3つでした。' }),
      h('div', { class: 'stack' }, [
        h('button', {
          class: 'btn btn-line', type: 'button',
          onclick: function () {
  ...
  function surfaceDetail() {
    ...
    return section('05-experience-detail', [
      ...
      h('h2', { class: 'section-title', text: '実際の情報' }),
      h('dl', { class: 'facts' }, [
        h('dt', { text: '時間' }), h('dd', { text: experience.duration }),
        h('dt', { text: '料金' }), h('dd', { text: experience.price })
      ]),
      h('p', { class: 'note', text: 'これはUX確認用のprototype dataです。実在の店舗・イベントではありません。' }),
