...
(function(){
 ...
 function select(events,{now=Date.now(),week=monday(now),city='',audience=''}={}){
  const today=date(now),end=add(week,7);
  return events.filter(e=>e.status==='scheduled'&&e.checkedAt<=today&&e.reviewThrough>=today&&(!city||e.city===city)&&(!audience||e.audiences.includes(audience)))
 ...
 function safeParams(p){const result=new URLSearchParams();const w=p.get('week');if(/^\d{4}-\d{2}-\d{2}$/.test(w||'')&&Number.isFinite(stamp(w))&&date(stamp(w))===w)result.set('week',w);if(data.cities[p.get('city')])result.set('city',p.get('city'));if(data.audiences.some(a=>a.id===p.get('with')))result.set('with',p.get('with'));return result;}
 ...
 const weekSelect=form.elements.week,citySelect=form.elements.city,withSelect=form.elements.with;
 ...
 options();const p=safeParams(query);if([...weekSelect.options].some(o=>o.value===p.get('week')))weekSelect.value=p.get('week');citySelect.value=p.get('city')||'';withSelect.value=p.get('with')||'';
