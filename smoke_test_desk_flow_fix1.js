...
async function main(){
  ...
  // 11. 入力済みタイトルが言語切替・助け舟(店主の助け舟ボタン)操作・再描画で消えない
  {
    ...
    ok('(B12c) 英語切替後も「いつの気持ちか」(whenSelect)の選択が残る', document.getElementById('whenSelect').value === 'past');
    window.toggleLanguage();
    ...
    ok('(B12e) 画面の出入り後も「いつの気持ちか」の選択が残る', document.getElementById('whenSelect').value === 'past');
  }
  ...
  // ============================================================
  {
    ...
    // 20. 助け舟（言葉にするための助け舟）が正常
    ok('(D20a) 助け舟の見出しが表示されている', document.getElementById('writingBoat').querySelector('.writing-boat-heading').textContent.length > 0);
