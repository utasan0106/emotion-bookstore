...
(function () {
  ...
  if (hero) hero.append(panel); else main.prepend(panel);
  ...
  async function refresh() {
    ...
    if (inFlight || document.hidden || Date.now() - lastAttempt < 60000) return;
    inFlight = true; lastAttempt = Date.now();
    try {
      const response = await fetch('/api/tokyo-weather', { credentials: 'omit', referrerPolicy: 'no-referrer', signal: AbortSignal.timeout(42000) });
