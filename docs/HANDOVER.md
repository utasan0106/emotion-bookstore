...
**(3) 共有されたときのカードを、索引163ページ全部に付けた。**

貼られたリンクが、カード画像も説明も無いただの文字列として出ていた。
`og:image` を持っていたのは3ページだけ、**催し43件には OGP が一つも無かった。**
10月の予約済み9稿は全稿がサイトへリンクするのに、着地先に扉が無かった。

扉は既に本番にある 1200x630 のブランド画像ひとつ。文言は書き起こしていない。
契約は `qa/social_card_check.js`（`verify-product` が回す）。
**表示回数は増えない。増えた表示がクリックになる形である。** 詳細は `REACH-20260914.md` §4-b。

**(4) 神田古本まつりが催し一覧に無い。10月分でいちばん大きな取りこぼし。**
