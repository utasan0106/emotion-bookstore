...
- **このブランチを `main` にマージする。** 週次の自走はそれまで動かない（§2）
- **X の予約投稿に紐づく制約**（`LAUNCH-COPY-202610.md` 冒頭）。10/31 の稿が
  作品数「77件」を名指ししている。**10/31 より前に作品を足すなら、先に予約を直す。**
  10/22 の神田古本まつりは、開催確認が取れていなければ投稿日までに取り消す
