...
### 事前承認された自走（2026-09-11、Founder 承認）

次は、都度の承認なしに進めてよい。**ただし成果は branch への push まで。
`main` には触れない。PR も作らない。**

- 催しの補充（採否基準を全部満たすもの）
- 再確認期限の延長（会場・主催者の公式ページで開催を確かめたもの）
- `hook` / `relation` の下書き（型の中で。掲載前に編集部が読む前提）
- テストの契約更新（実態に合わせる場合）、実装・リファクタ・回帰修正
- 検知・監視を強める道具の追加

採否基準・文言の型・越えてはいけない線は
`docs/strategy/AUTONOMY-20260911.md` にある。**自走する前に読む。**
同ファイルは上の Guardrails を狭める方向にしか働かない。矛盾したら本ファイルが勝つ。

判断がつかない、基準に無い、権利が絡む、公開済みの事実を動かすことになる——
そのときは push せずに止めて報告する。
