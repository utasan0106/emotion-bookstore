...
for(const essay of research) {
  ...
  const media=essay.mediaIntro ? `<section class="research-media" id="research-media"><h2>${esc(essay.mediaIntro.title)}</h2><p>${esc(essay.mediaIntro.text)}</p><p>${external(sources.get(essay.mediaIntro.source).url,'自治体の映像紹介ページを開く')}</p></section>` : `<section class="research-media" id="research-media"><p class="eyebrow">広告を実物で見る</p><h2>映像と文章を、並べて読む。</h2>${visibleMedia}<details class="background"><summary>広告画像の扱い</summary><p>${esc(essay.mediaPolicy.visible)}</p><p>${esc(essay.mediaPolicy.linked)}</p><p>${esc(essay.mediaPolicy.withheld)}</p></details></section>`;
