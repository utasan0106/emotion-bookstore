...
for(const essay of research) {
  ...
  const sections=essay.sections.map(section=>`<section><p class="eyebrow">${esc(section.kind)}</p><h2>${esc(section.title)}</h2><p>${esc(section.text)}</p>${section.source?`<p><a href="#source-${esc(section.source)}">この記述の出典と確認範囲 →</a></p>`:''}</section>`).join('');
