...
for(const essay of research) {
  ...
  const media=`<section class="research-media"><p class="eyebrow">広告を実物で見る</p><h2>映像と文章を、並べて読む。</h2>${visibleMedia}<details class="background"><summary>広告画像の扱い</summary><p>${esc(essay.mediaPolicy.visible)}</p><p>${esc(essay.mediaPolicy.linked)}</p><p>${esc(essay.mediaPolicy.withheld)}</p></details></section>`;
