...
const researchCards=research.map(essay=>`<article class="research-card"><p class="eyebrow">${esc(essay.issue)} · ${esc(essay.cityLabel)}</p><h2><a href="/discover/essays/${essay.id}.html">${esc(essay.title)}</a></h2><p>${esc(essay.lead)}</p><ul aria-label="調査の視点">${essay.lenses.map(lens=>`<li>${esc(lens)}</li>`).join('')}</ul><p class="research-meta">確認済み${essay.sources.length}資料 · 更新 ${esc(essay.modifiedAt)}</p><a class="research-link" href="/discover/essays/${essay.id}.html">調査を読む →</a></article>`).join('');
write('essays/index.html',shell('街を多角的に読む',`<section class="intro research-intro"><p class="eyebrow">AIで街を多角的に読む</p><h1>一つの街を、<br>一つの答えにしない。</h1><p class="lead">広告、来街調査、建築、作品、人、街同士のつながり。異なる時代と種類の資料を照合し、確認できる事実と編集上の問いを分けて読みます。</p></section><section class="research-grid" aria-label="公開中の街の調査">${researchCards}</section><aside class="research-method"><h2>この調査で守ること</h2><p>出典の数を結論の強さに置き換えません。広告主の意図、調査結果、編集上の解釈を区別し、分からないことも記事に残します。</p></aside>`,'<a href="/discover/">街と作品へ戻る</a>'));
for(const essay of research) {
  const sources=new Map(essay.sources.map(source=>[source.id,source]));
  const sections=essay.sections.map(section=>`<section><p class="eyebrow">${esc(section.kind)}</p><h2>${esc(section.title)}</h2><p>${esc(section.text)}</p>${section.source?`<p>${external(sources.get(section.source).url,sources.get(section.source).title)}</p>`:''}</section>`).join('');
  ...
  const media=`<section class="research-media" id="research-media"><p class="eyebrow">広告を実物で見る</p><h2>映像と文章を、並べて読む。</h2>${visibleMedia}<details class="background"><summary>広告画像の扱い</summary><p>${esc(essay.mediaPolicy.visible)}</p><p>${esc(essay.mediaPolicy.linked)}</p><p>${esc(essay.mediaPolicy.withheld)}</p></details></section>`;
  const ledger=essay.sources.map(source=>`<section><h3>${esc(source.title)}</h3><p>${esc(source.publisher)} · ${esc(source.type)}<br>発表：${esc(source.publishedAt)} / 対象：${esc(source.period)}</p><p>${esc(source.limitation)}</p>${external(source.url,'発表元の資料を開く')}</section>`).join('');
  const comparison=`<section id="research-comparison"><h2>広告・販促企画を4つの入口から読む</h2><p>以下は当時の告知内容の整理です。すべて終了した企画で、現在のイベント案内ではありません。</p>${essay.comparisons.map(row=>`<details class="background"><summary>${esc(row.when)} · ${esc(row.person)}</summary><dl><dt>街との関係</dt><dd>${esc(row.relation)}</dd><dt>誘っている行動</dt><dd>${esc(row.action)}</dd><dt>ここからは分からないこと</dt><dd>${esc(row.missing)}</dd></dl><p>${external(sources.get(row.source).url,'当時の広告・発表画像を見る')}</p></details>`).join('')}</section>`;
  const timeline=`<section id="research-timeline"><h2>高校生のころから、何が変わった？</h2><p>2026年の30代の高校時代は、おおむね2000年代〜2010年代前半にまたがります。今の年齢ではなく、自分が通っていた年を手掛かりに読んでください。現段階では2010年以降の出来事を掲載し、2000年代の資料・当事者の声は未収集です。</p><ol>${essay.timeline.map(row=>`<li><h3>${esc(row.year)} · ${esc(row.title)}</h3><p>${esc(row.text)}</p><p>${external(sources.get(row.source).url,'根拠の資料')}</p></li>`).join('')}</ol><p>街そのものの変化と、自分が高校生から大人になったことで変わった行動は別です。「昔のほうが良かった」「若者が来なくなった」といった評価は、この年表からは導けません。</p></section>`;
  write(`essays/${essay.id}.html`,shell(essay.title,`<article class="detail"><p class="eyebrow">${esc(essay.series)} · ${esc(essay.issue)}</p><p class="research-back"><a href="/discover/essays/">街の調査一覧</a> / ${esc(essay.cityLabel)}</p><h1>${esc(essay.title)}</h1><p class="detail-hook">${esc(essay.lead)}</p><p class="research-byline">みんなの感情書店編集部 · 更新 ${esc(essay.modifiedAt)} · 確認済み${essay.sources.length}資料</p>${media}<nav class="research-toc" aria-label="記事の目次"><a href="#research-comparison">広告を比べる</a><a href="#research-timeline">街の変化を辿る</a><a href="#research-sources">調べ方・出典</a></nav>${comparison}${timeline}${sections}<section><h2>今度は、映像の吉祥寺へ。</h2><p>広告の中の街と見比べたら、映画や街の映像では何が目に留まるだろう。</p><p><a href="/discover/kichijoji/film.html">吉祥寺とつながる映画を見る →</a></p><p><a href="/discover/kichijoji/video.html">街の映像を選ぶ →</a></p></section><details class="background" id="research-sources"><summary>調べ方と出典：確認済み${essay.sources.length}資料</summary><p>AIを資料探索・比較・文章化に使用した小規模な試読版です。大規模調査や現地取材ではありません。広告主の意図、調査結果、編集上の解釈を分けています。広告の静止画・コピー・インタビュー本文は転載していません。本人・関係団体の監修や公認を受けた記事ではありません。</p>${ledger}<p>出典確認：${esc(essay.checkedAt)}</p></details></article>`,'<a href="/discover/essays/">街の調査へ戻る</a>'));
}
function workCard(i) {
...
const researchSpotlight=`<section class="research-spotlight" aria-labelledby="research-spotlight-title"><div><p class="eyebrow">${esc(research[0].series)} · 最新</p><h2 id="research-spotlight-title"><a href="/discover/essays/${research[0].id}.html">${esc(research[0].title)}</a></h2><p>${esc(research[0].lead)}</p></div><div><p class="research-meta">${esc(research[0].cityLabel)} · 確認済み${research[0].sources.length}資料</p><a class="research-link" href="/discover/essays/${research[0].id}.html">最新の調査を読む →</a><a class="research-link" href="/discover/essays/">街の調査一覧 →</a></div></section>`;
write('index.html', shell('街から音楽、映像、本、映画を探す', `<section class="intro"><p class="eyebrow">聴く・観る・読む</p><h1>街から、<br>気になる作品へ。</h1><p class="lead">街で鳴った音楽、街を映す映像、本、映画に出会う文化案内。<br>まずひとつ楽しんで、ゆかりの場所や関連特集へ。</p></section>${watchNow}${researchSpotlight}
