...
function write(file, html) {
  ...
  if(detailItem) {
    ...
    // その場所に他にも作品や催しがあるなら、そこへ抜けられるようにする。
...
// 街ページから会場へ。場所を軸にした横断は、街の一覧からも入れないと見つからない。
...
// A dated observation joins two differently named shopping streets to an existing book.
// No Mapillary image is downloaded or embedded; the photo remains at its source.
function koenjiStreetFragment() {
  const scene='538363923973908';
  const imagePage=`https://www.mapillary.com/app/?pKey=${scene}`;
  return `<section class="street-fragment" aria-labelledby="street-fragment-title"><div class="street-fragment-main"><p class="eyebrow">街の断片 · 2015年の記録</p><h2 id="street-fragment-title">庚申通りの名前をたどる</h2><p>2015年の写真に写る庚申通り。商店街が伝えるその名の背景には、1716年に建てられた庚申塔があります。高円寺には、小説の題名から愛称が生まれた別の「純情商店街」も。二つの名前から、この街と一冊の本をたどってみませんか。</p><a class="street-fragment-action" href="/discover/koenji/junjo.html">ねじめ正一『高円寺純情商店街』を見る →</a></div><div class="street-fragment-sources"><p>庚申塔はこの写真には写っていません。店や設備の現在の状態を示す写真ではありません。</p><a class="street-fragment-mapillary" href="https://www.mapillary.com/?locale=ja_JP" target="_blank" rel="noopener noreferrer"><img src="https://michiyomi.dev/explore/vendor/mapillary-logo.svg" alt="Mapillary" width="112" height="28" loading="lazy" decoding="async">Mapillary</a><details><summary>写真・名前の出典</summary><p><a href="${imagePage}" target="_blank" rel="noopener noreferrer">2015年の元写真をMapillaryで見る ↗</a> · <a href="https://michiyomi.dev/explore/" target="_blank" rel="noopener noreferrer">みちよみの観測</a>（scene ${scene} / snapshot 2026-09-13-r1）</p><p><a href="https://koushindoori.com/about/mame1" target="_blank" rel="noopener noreferrer">庚申通り商店街による庚申塔の解説 ↗</a> · <a href="https://www.kouenji.or.jp/event/9871.html" target="_blank" rel="noopener noreferrer">純情商店街による小説と愛称の説明 ↗</a></p><p>みちよみの観測データは © Mapillary contributors（<a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noopener noreferrer">CC BY-SA 4.0</a>）に基づきます。写真の転載はしていません。</p></details></div></section>`;
}
...
const signalSource=require('./weekly-outings-source'),signalWeek=require('../outings/week'),signalToday=signalWeek.date(Date.now());
const signalEvents=signalSource.events.filter(e=>signalSource.isPublishableEvent(e)&&e.status==='scheduled'&&e.checkedAt<=signalToday&&e.reviewThrough>=signalToday&&signalWeek.dates(e).at(-1)>=signalToday);
const activeSignals=citySignals.flatMap(signal=>{const event=signalEvents.find(e=>e.id===signal.event);return event?[{signal,event}]:[];});
const signals=activeSignals.length?`<section class="quick" id="city-signals"><h2>今の街の動き</h2><p>編集部が現在確認できた${activeSignals.length}つの文化企画。日程と参加条件は、それぞれの案内で確認できます。</p>${activeSignals.map(({signal,event})=>{const dates=event.dates||[event.start,event.end];return `<article><p class="eyebrow">${cityNames[event.city]} · 告知された開催期間 ${esc(dates[0])}〜${esc(dates.at(-1))}</p><h3>${esc(signal.title)}</h3><p>${esc(event.title)}</p><p>編集部の視点：${esc(signal.reading)}</p><p><a href="/outings/events/${event.id}.html">日程・参加条件を見る</a> · ${external(event.url,'公式告知')}</p></article>`;}).join('')}<p>開催変更・空席は公式案内をご確認ください。自動更新や人気ランキングではありません。</p><a href="/outings/">開催週を選んで探す →</a></section>`:'';
write('index.html', shell('東京5街から本・映画・音楽・文化を探す', `<section class="intro"><p class="eyebrow">聴く・観る・読む・出かける</p><h1>街から探す</h1><p class="lead">高円寺・下北沢・吉祥寺・神保町・清澄白河から、作品や場所との出会いを選ぶ。</p></section>
<section class="city-grid" aria-label="街を選ぶ">${cities.map(c=>`<a class="city-card" href="/discover/${c}/"><div class="city-image">${photo(c)}</div><div class="city-caption"><h2>${cityNames[c]}</h2><p>音楽 ${items.filter(i=>i.city===c&&i.kind==='audio').length} / 映像 ${items.filter(i=>i.city===c&&i.kind==='video').length} / 本・漫画 ${items.filter(i=>i.city===c&&i.kind==='book').length} / 映画 ${items.filter(i=>i.city===c&&i.kind==='film').length}</p><span>${cityNames[c]}の作品を選ぶ →</span></div></a>`).join('')}<a class="city-card" href="/discover/kiyosumi/"><div class="city-image">${photo('kiyosumi')}</div><div class="city-caption"><h2>清澄白河</h2><p>庭園 / 美術館 / 江戸文化</p><span>清澄白河の場所・作品を選ぶ →</span></div></a></section>${watchNow}${signals}${researchSpotlight}
...
// 会場から辿る。街 → 分類 → 作品 → 外へ、という一直線では見えない交差を出す。
// 同じ部屋で録られた演奏と、その部屋を書いた本と、今週その部屋でやっている催しは、
// 読者にとって一つのものである。会場は会期で消えないので、この索引は空にならない。
{
  ...
  for (const v of verifyVenues(items)) {
    ...
    const byKind = {};
    for (const w of works) (byKind[w.kind] = byKind[w.kind] || []).push(w);
    const workSections = Object.entries(byKind).map(([kind, group]) =>
      `<section class="wk-list" aria-label="${categories[kind].name}${group.length}件"><h2>${categories[kind].name}</h2><ul>${
    ...
    const eventSection = here.length
      ? `<section class="wk-list" aria-label="${v.name}のいまの催し"><h2>いま、ここで <small data-venue-events-count></small></h2><ul data-venue-events>${eventRows}</ul><p data-venue-events-empty hidden>いま予定されている催しはありません。<a href="/outings/?city=${v.city}">${venueCities[v.city]}の催しを見る →</a></p></section>`
      : `<section class="wk-list" aria-label="${v.name}のいまの催し"><h2>いま、ここで</h2><p>この会場の催しは、いまカタログにありません。<a href="/outings/?city=${v.city}">${venueCities[v.city]}の催しを見る →</a></p></section>`;
    write(`venue/${v.id}/index.html`, shell(`${v.name}｜${venueCities[v.city]}`,
...
require('node:child_process').execFileSync(process.execPath,[path.join(__dirname,'refresh-sitemap.js'),...(process.argv.includes('--check')?['--check']:[])],{cwd:root,stdio:'inherit'});
