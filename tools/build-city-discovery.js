...
for(const essay of research) {
  ...
  const sections=essay.sections.map(section=>`<section><p class="eyebrow">${esc(section.kind)}</p><h2>${esc(section.title)}</h2>${(section.paragraphs||[section.text]).map(p=>`<p>${esc(p)}</p>`).join('')}${section.source?`<p><a href="#source-${esc(section.source)}">出典を確認 →</a></p>`:''}</section>`).join('');
  ...
  const ledger=essay.sources.map(source=>`<section id="source-${esc(source.id)}" class="research-source"><h3>${esc(source.title)}</h3><p>${esc(source.publisher)} · ${esc(source.type)}<br>発表：${esc(source.publishedAt)} / 対象：${esc(source.period)}</p><p>${esc(source.limitation)}</p>${external(source.url,'発表元の資料を開く')}<p><a href="#research-reading">本文に戻る ↑</a></p></section>`).join('');
