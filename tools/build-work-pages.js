...
const catalogueKind={book:'book', film:'film', music:'audio'};
...
const featuredCatalogueId={book:'jinbocho/kaijin', film:'jinbocho/morisaki-film'};
...
function kindCount(entryId){
  ...
  const label={book:'本', film:'映画', music:'音楽'}[entryId];
...
const mediaFor=entry=>entry.id==='music'?media.album():entry.id==='film'?media.youtube('6M0vx8wLEbM',entry.title,'予告編（本編ではありません）'):media.cover('jinbocho/kaijin',entry.title,BOOK_OFFICIAL);
const entries = [
...
const worksHeader=seoHeader(header,{
  title:'東京の本・映画・音楽｜みんなの感情書店',
  description:'高円寺・下北沢・吉祥寺・神保町にゆかりのある本・映画・音楽を、街との関係から選べる文化案内。',
...
` + footer);
const retired=path.join(root,'work-video.html');
if(process.argv.includes('--check')) {
  if(fs.existsSync(retired)) throw new Error('Retired overlong video page still exists: work-video.html');
} else if(fs.existsSync(retired)) fs.unlinkSync(retired);
console.log('PASS work directory + 3 separate pages' + (process.argv.includes('--check') ? ' match source' : ' generated'));
